...
import './style.css';
import $ from 'jquery';
import { ipc, getLocale, closeWindow, readTextFile, resolveResourcePath, importBat, exportBat } from './lib/tauri-bridge.js';
...
$('#export').click(async function () {
    try {
        await exportBat(data().table);
    } catch (err) {
        console.error('export bat failed', err);
    }
});

$('#import').click(async function () {
    try {
        const res = await importBat();
        if (res && res.table) {
            chart.series[0].setData([]);
            $('#Cutoff').val(res.table.Cutoff);
            res.table.PercentsVoltage.reverse().forEach((p, i) => {
        ...
        console.error('import bat failed', err);
    }
});
